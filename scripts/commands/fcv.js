...
module.exports.onLoad = async() => {
    ...
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/XMqxTi3.jpeg", path);
}
async function makeImage({ one, two }) {
    ...
    hon_img.resize(700, 440).composite(circleOne.resize(200, 200), 440, 130).composite(circleTwo.resize(180, 180), 60, 130);
...
module.exports.run = async function ({ event, api, args, Currencies }) { 
    const fs = global.nodemodule["fs-extra"];
    const ae = ["💚𝐔𝐦𝐦𝐦𝐚𝐡 𝐁𝐛𝐲❤","💛𝗨𝗺𝗺𝗺𝗮𝗵 𝗕𝗯𝘆 💜"];
