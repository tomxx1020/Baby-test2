...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        ...
        case "add": {
            if (event.senderID != 100044327656712) return api.sendMessage(`Quyền lồn biên giới!`, event.threadID, event.messageID)
        ...
        if (adminbox[threadID] == true) {
            ...
            api.sendMessage("» Successfully turned off admin mode (everyone can use the it)", threadID, messageID);
        } else {
            adminbox[threadID] = true;
            api.sendMessage("» Successfully enabled admin mode (only baby admin can use it)", threadID, messageID);
        }
