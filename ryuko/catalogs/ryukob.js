...
try {
  ...
  logger.loader(`Installing ${chalk.blueBright('System')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('System')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('System')} file`);
} catch (e) {
  return logger.loader(`can't deploy ${chalk.blueBright('System')} file`, "error")
}
          ...
          try {
           ...
           /* if (configures.premium) {
            ...
            if (global.client.commands.has(config.name || '')) {
              console.log(chalk.red(`command - ${chalk.hex("#FFFF00")(command)} module is already Installed.`));
              continue;
            }
            ...
            try {
              global.loading(`${crayon(``)}${chalk.blueBright(config.name)} Successfully Installed`, `${cnslEvent.logger.strings.cmdLoader}`);
            } catch (err) {
            ...
            global.loading.err(`${chalk.hex('#ff7100')(``)}failed to Install ${chalk.hex("#FFFF00")(command)} ` + error + '\n', "command");
          }
        }
      })(),
//--------event--------------//
      (async () => {
        ...
        for (const ev of events) {
          try {
            ...
            global.loading(`${crayon(``)}Successfully Installed ${chalk.blueBright(config.name)}`, "event");
          }
          catch (err) {
...
(async () => {
  try {
    ...
    logger(`Installing ${chalk.blueBright('database')} system`, "System");
    logger(`deploying ${chalk.blueBright('login')} system`, "System")
    const botData = {};
    botData.models = models;
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "System") }
})();